...
class TestBowling(scrapy.Spider):
    name = "TestBowling"
 
    def start_requests(self):       #pass the url that the table we need belongs to
        urls = [
            'http://stats.espncricinfo.com/sl/engine/records/averages/bowling.html?class=1;current=2;id=8;type=team',
        ]
        for url in urls:
            yield scrapy.Request(url=url, callback=self.parse)      #save the url and passes to callback function again
    ...
    def parse(self, response):
        for row in response.xpath('//*[@class="engineTable"]//tbody/tr'):       #getting data from espn stat records ODI bowling line by line
            ...
            yield {
                ...
                'Year Span' : row.xpath('td[2]//text()').extract_first(),           #resuest the text values in the table data cells
                ...
                'Maidens' : row.xpath('td[6]//text()').extract_first(),
        ...
        #final outcome comes as a json file when the command given in the prompt