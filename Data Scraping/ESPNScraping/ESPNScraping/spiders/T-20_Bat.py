...
class T_20_Batting(scrapy.Spider):
    name = "T_20_Batting"
 
    def start_requests(self):       #pass the url that the table we need belongs to
        urls = [
            'http://stats.espncricinfo.com/sl/engine/records/averages/batting.html?class=3;current=2;id=8;type=team',
        ]
        for url in urls:
    ...
    def parse(self, response):
        for row in response.xpath('//*[@class="engineTable"]//tbody/tr'):   #getting data from espn stat records ODI bowling line by line
            ...
            yield {
                ...
                'NO' : row.xpath('td[5]//text()').extract_first(),
                'Runs' : row.xpath('td[6]//text()').extract_first(),
                'Highest Score' : row.xpath('td[7]//text()').extract_first(),       #resuest the text values in the table data cells
