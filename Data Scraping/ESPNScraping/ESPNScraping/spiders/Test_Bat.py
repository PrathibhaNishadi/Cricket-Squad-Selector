...
class TestBatting(scrapy.Spider):
    name = "TestBatting"
 
    def start_requests(self): #pass the url that the table we need belongs to
        urls = [
            'http://stats.espncricinfo.com/sl/engine/records/averages/batting.html?class=1;current=2;id=8;type=team',
        ]
        for url in urls:
    ...
    def parse(self, response):
        for row in response.xpath('//*[@class="engineTable"]//tbody/tr'):       #getting data from espn stat records ODI bowling line by line
            ...
            yield {
                ...
                'No of Matches' : row.xpath('td[3]//text()').extract_first(),       #resuest the text values in the table data cells
